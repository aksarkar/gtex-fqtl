...
** Develop TWAS implementation
   ...
   #+BEGIN_SRC ipython
     if __name__ == '__main__':
       # Important: this doesn't get closed properly, so we need to open it here
       with tabix.open('/broad/hptmp/aksarkar/gtex-gwas-hg19/imputed_IGAP_Alzheimer.bed.gz') as f:
         for k, row in genes:
           qtl_b, qtl_cov = load_fqtl_model(f'/broad/compbio/ypp/gtex/analysis/fqtl-gtex/result/fqtl-std/{k}/50/fqtl')
           gwas_z = pd.DataFrame(f.query(f'chr{row[2]}', row[3] - 1e6, row[3] + 1e6))
   #+END_SRC
