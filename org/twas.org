...
** APOE example
   ...
   \begin{align*}
     ...
     &= (\vb' \mx' \mx \vb)^{-1} \vb' \mx' \vy, \qquad \mx = \mU\md\mv', \mU'\mU = \mi, \mv'\mv = \mi\\
     &= (\vb' \mv' \md^2 \mv \vb)^{-1} \vb' \vz
   \end{align*}
   ...
   #+BEGIN_SRC ipython :async t
     ...
     stat = pd.Series(z @ B_thresh / (X.shape[0] * np.diag(B_thresh.T @ R @ B_thresh) + 1e-8), index=pip.columns)
     perm_stat = []
     ...
     for t in B:
       ...
       den = X.shape[0] * (R.values @ bt.power(2)).sum(axis=0, keepdims=True)
   ...
   #+BEGIN_SRC ipython :ipyfile figure/twas.org/apoe.png
     ...
     ax[0].bar(grid, stat, width=.9, color='k', zorder=3)
     ax[0].set_ylabel('TWAS statistic')
     ax[1].bar(grid, perm_logp, width=.9, color='k', zorder=3)
     for a in ax:
       a.set_xticks(grid)
       a.axhline(y=0, lw=1, color='0.7')
